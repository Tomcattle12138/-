...
class TimeGAN(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.feature_dim = args.feature_dim  # 特征向量维度
        self.Z_dim = args.Z_dim
    ...
    def _discriminator_forward(self, X, T, Z, gamma=1):
        ...
        # Forward Pass
        Y_real = self.discriminator(H, T)            # Encoded original data  判断真实数据是真实数据
        Y_fake = self.discriminator(H_hat, T)        # Output of generator + supervisor  判断
        Y_fake_e = self.discriminator(E_hat, T)      # Output of generator  判断生成数据是假的
